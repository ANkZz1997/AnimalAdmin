...
export default function KycOption() {
  ...
  const saveNewDocType = async () => {
    ...
    try {
      const res = await axios.post(
        `${URLS.EXCHANGE.ADMIN.ADD_KYC_DOC_LIST}`,
        data
      );
  ...
  const changeStatusDoc = async (val, id) => {
    ...
    try {
      const res = await axios.post(
        `${URLS.EXCHANGE.ADMIN.UPDATE_KYC_DOC}`,
        data
      );
  ...
  return (
    <Root>
      <h3>
        You can change the KYC document settings.
        <button
          onClick={() => {
            setAddPop(true);
          }}
        ...
        <div className="doc_child">
          {getDoc &&
            getDoc?.map((i, ix) => {
              return (
                <div
                  className={i.enabled ? "doc_body" : "doc_body no"}
                  key={ix}
                >
                  <button
                    onClick={() => {
                      changeStatusDoc(!i.enabled, i.id);
                    }}
                    className="act_btn"
                  >
                  ...
                  <div>
                    ...
                    <p>{i?.enabled ? "Enabled" : "Disabled"}</p>
                  </div>
                  <div>
                    <h5>Id Proof</h5>
                    <p>{i?.identitiyProof ? "True" : "False"}</p>
                  </div>
                  <div>
                    <h5>Address Proof</h5>
                    <p>{i?.addressProof ? "True" : "False"}</p>
                  </div>
                  <div>
                    <h5>Created On</h5>
                    <p> {`${moment(i?.createdAt).format("DD-MMM-YY")}`}</p>
                  </div>
                  <div>
                    <h5>Updated on</h5>
                    <p>{`${moment(i?.updatedAt).format("DD-MMM-YY")}`}</p>
                  </div>
                </div>
              );
            })}
      ...
      <div className={addPop ? "add_doc" : "add_doc no"}>
        <div className="add_doc_box">
          ...
          <div>
            <button
              className={!identity && !address ? "btn_hide" : " "}
              onClick={() => {
                saveNewDocType();
              }}
            >
